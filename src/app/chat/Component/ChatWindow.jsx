...
import { Avatar, Badge, Button, Form, Input, Upload, Image } from 'antd';
...
const ChatWindow = ({ id }) => {
    ...
    const users = chatList?.find(chat => chat?.participants?.map(item => item === id));
    console.log(users?.participants) // array return 
            ...
            <div className="p-4 bg-white flex items-center gap-4 border-b border-gray-200">
                {users?.participants?.map((participantId, index) => (
                    <div key={participantId} className="relative">
                        <Avatar
                            src={`${baseURL}${participantId?.profile}` || "/images/default-avatar.jpg"}
                            size={40}
                        />
                        <Badge
                            style={{
